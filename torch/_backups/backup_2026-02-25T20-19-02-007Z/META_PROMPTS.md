...
- If `torch/scheduler-flow.md` exists in the current working directory, run in TORCH-root mode:
  - `script_prefix = npm run`
  - `path_prefix = ` (empty)
- Else if `torch/torch/scheduler-flow.md` exists, run in host-repo mode:
...
- prompt_dir = <path_prefix>torch/prompts/daily/
...
1) test -f <path_prefix>torch/TORCH.md && cat <path_prefix>torch/TORCH.md || echo "No torch/TORCH.md found; continuing" (missing torch/TORCH.md is non-fatal)
...
4) Select next roster agent using this exact algorithm:
   ...
   - Resolve the roster path as `<path_prefix>torch/roster.json`.
...
7) Execute selected prompt from <path_prefix>torch/prompts/daily/ (spawned agent work only; completion publish and final task log writing remain scheduler-owned)
...
- prompt_dir = <path_prefix>torch/prompts/weekly/
...
7) Execute selected prompt from <path_prefix>torch/prompts/weekly/ (spawned agent work only; completion publish and final task log writing remain scheduler-owned)
8) After execution, run the storage command if configured:

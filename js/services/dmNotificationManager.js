...
export class DmNotificationManager {
  ...
  enqueueUpdate(conversationId, task) {
    const key = conversationId || "global";
    const previous = this.updateQueue.get(key) || Promise.resolve();
    const next = previous
      .then(() => task())
      .catch((error) => {
        this.logger.warn(
          `[dmNotifications] Update task failed for ${key}:`,
          error,
        );
      });
    this.updateQueue.set(key, next);
    return next;
  }
  ...
  recordMessage({ record, preview = "", direction = "" } = {}) {
    ...
    return this.enqueueUpdate(conversationId, async () => {
      ...
      const normalizedUpdate = normalizeConversationState({
        ...
        opened_until: updated?.opened_until ?? conversationState.opened_until,
      });
  ...
  acknowledgeRenderedMessages({ conversationId, renderedUntil } = {}) {
    if (typeof conversationId !== "string" || !conversationId.trim()) {
      return Promise.resolve(null);
    }
    ...
    return this.enqueueUpdate(normalized, async () => {

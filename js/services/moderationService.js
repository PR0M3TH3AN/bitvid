...
import {
  ...
  normalizeEventId,
  normalizeReportType,
  normalizeMuteCategory,
  extractMuteCategoryFromTag,
  cloneSummary,
  encodeToNpub,
  normalizeToHex,
  ...
  ensureNumber,
  resolveStorage,
  AUTOPLAY_TRUST_THRESHOLD,
  BLUR_TRUST_THRESHOLD,
  TRUSTED_MUTE_WINDOW_SECONDS,
  decodeToHex
...
export class ModerationService {
  ...
  /**
   * Batch retrieval of moderation info for multiple authors.
   * Optimized to prune aggregates in one pass.
   *
   * @param {string[]} authors - List of author pubkeys.
   * @returns {Map<string, object>} Map of author -> { trustedMuted, trustedMuters, trustedMuteCountTotal, trustedMuteCountsByCategory }
   */
  getModerationInfoForAuthors(authors) {
    ...
    } else if (authors instanceof Set) {
      for (const author of authors) {
        const normalized = normalizeHex(author);
        if (normalized) {
          uniqueAuthors.add(normalized);
    ...
    this.pruneTrustedMuteAggregates(Array.from(uniqueAuthors));

    for (const author of uniqueAuthors) {
      const entry = this.trustedMutedAuthors.get(author);
      let trustedMuted = false;
      let trustedMuters = [];
      let trustedMuteCountTotal = 0;
      let trustedMuteCountsByCategory = null;

      if (entry && entry.muters instanceof Map && entry.muters.size > 0) {
        trustedMuted = true;
        trustedMuters = Array.from(entry.muters.keys());
        trustedMuteCountTotal = entry.muters.size;

        if (entry.categories instanceof Map) {
          trustedMuteCountsByCategory = {};
          for (const [category, muters] of entry.categories.entries()) {
            if (typeof category === "string" && category.trim()) {
              trustedMuteCountsByCategory[category] = muters.size;
            }
          }
        }
      }

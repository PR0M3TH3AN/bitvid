...
import { applyAdminStar, applyAdminRing } from "../../adminBadge.js";
...
export class CommentNodeFactory {
  ...
  createCommentNode(event, { depth = 0 } = {}) {
    ...
    // Gold admin ring + star for comments authored by a bitvid admin.
    if (typeof event.pubkey === "string" && event.pubkey.trim()) {
      const authorKey = event.pubkey.trim();
      applyAdminRing(avatarWrapper, authorKey);
      applyAdminStar(avatarWrapper, authorKey, { doc: this.document });
    }

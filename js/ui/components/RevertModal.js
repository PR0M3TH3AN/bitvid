    ...
    return `<ol class="space-y-3">${variants
      .map((variant, index) => {
        ...
        if (variant.autoGenerated) {
          badges.push(
            '<span class="inline-flex items-center rounded-full border border-amber-600/70 bg-amber-900/30 px-2 py-0.5 text-4xs uppercase tracking-wide text-amber-200/90">Auto-generated</span>'
          );
        }
        ...
        const renderNestedList = (label, values, emptyLabel) => {
          if (!Array.isArray(values) || values.length === 0) {
            return `<div><p class="text-2xs uppercase tracking-wide text-gray-500">${escape(
              label
          ...
          return `
            <div>
              <p class="text-2xs uppercase tracking-wide text-gray-500">${escape(
                label
              )}</p>
    ...
    return `<ol class="space-y-3">${tracks
      .map((track, index) => {
        ...
        const languageHtml = track.language
          ? `<span class="inline-flex items-center rounded-full border border-gray-700 bg-gray-800/70 px-2 py-0.5 text-2xs uppercase tracking-wide text-gray-200">${escape(
              track.language
            )}</span>`
          : this.renderPlaceholder();

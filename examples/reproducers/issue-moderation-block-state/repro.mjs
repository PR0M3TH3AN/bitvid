...
test("VideoCard block action restores trusted mute hide state after override", async (t) => {
  ...
  // This is where it fails in the original test
  assert.equal(contextAfterHide.overrideActive, false, "Override should be cleared after block");
  assert.equal(contextAfterHide.activeHidden, true, "Content should be hidden again after block");
  assert.equal(card.getRoot().dataset.moderationHidden, "true", "Card should have hidden dataset");
  assert.equal(card.moderationBlockButton, null, "Block button should be gone");
});

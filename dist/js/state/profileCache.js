...
class ProfileCache {
  ...
  getProfileData(pubkey, section) {
    ...
    if (this.memoryCache.has(pubkey)) {
      const userCache = this.memoryCache.get(pubkey);
      if (userCache.has(section)) {
    ...
    if (tier === STORAGE_TIERS.LOCAL_STORAGE && typeof localStorage !== "undefined") {
      try {
        ...
        if (raw) {
          ...
          if (!this.memoryCache.has(pubkey)) {
            this.memoryCache.set(pubkey, new Map());
          }
          this.memoryCache.get(pubkey).set(section, parsed);
          return parsed;
        }
      } catch (error) {
  ...
  setProfileData(pubkey, section, data) {
    ...
    this.memoryCache.get(pubkey).set(section, data);
  ...
  setMemoryData(section, data) {
    ...
    if (!this.memoryCache.has(this.activePubkey)) {
      this.memoryCache.set(this.activePubkey, new Map());
    }
    this.memoryCache.get(this.activePubkey).set(section, data);
  }

  getMemoryData(section) {
    if (!this.activePubkey) return null;
    if (this.memoryCache.has(this.activePubkey)) {
      return this.memoryCache.get(this.activePubkey).get(section);
    }
    return undefined;
  }

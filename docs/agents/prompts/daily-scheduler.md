...
Review the titles in the output. Every agent name that appears in a PR title containing `[daily]` is **OFF LIMITS** — do not select it.
...
|-------|-----------|---------|
| **Pre-flight gate** | Scan ALL open `[daily]` PRs via `curl` | Agents claimed by any platform |

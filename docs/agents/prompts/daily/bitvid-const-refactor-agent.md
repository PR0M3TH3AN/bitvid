You are: **bitvid-const-refactor-agent**, a senior software engineer AI agent working inside the `PR0M3TH3AN/bitvid` repo (target branch: `unstable`).
...
- For each duplicate:
  - Decide a canonical home (prefer existing `js/constants.js`, `js/nostr/relayConstants.js`, `js/nostr/cachePolicies.js`).
  - If canonical exists, replace duplicates by importing that constant.
  - If not, add a semantically-named constant to the best shared module (`js/constants.js` for app-wide, `js/nostr/*` for nostr concerns, otherwise `js/constants/<domain>.js`) and import it.
- Make small, safe edits, run linters and unit tests, and open a PR per change/related group.
- Success: repo passes `npm run lint` and `npm run test:unit`; each PR is atomic and has `CONTEXT.md`, `TODO.md`, `DECISIONS.md`, `TEST_LOG.md`, and a clear QA plan.

===============================================================================
HARD CONSTRAINTS / SAFETY
- Target branch is **`unstable`**. Do not operate against other branches unless instructed.
...
- `CONTEXT.md` — Why this refactor run exists and what it touches.
- `TODO.md` — List of duplicated constants to canonicalize and status for each.
- `DECISIONS.md` — Where canonical constants were placed and why. Record alternatives considered.
- `TEST_LOG.md` — Exact commands run (lint/tests) and outputs.
...
1) **Checkout & baseline**
   - Ensure you're on `unstable` and up to date:
     ```
   ...
   - Record baseline:
     ...
     - `node -v`, `npm -v`
     - Add this metadata to `CONTEXT.md`.
...
3) **Prioritize candidate groups**
   ...
   - Produce a candidate entry in `perf/constants-refactor/candidates.json`:
     ```
     {
       ...
       "occurrences": [
         {"file": "js/relayManager.js", "line": 24, "context":"FAST_RELAY_TIMEOUT_MS"},
         {"file": "js/nostr/watchHistory.js", "line": 10, "context":"FAST_PROFILE_TIMEOUT_MS"},
         ...
       ]
     }
     ```
...
4) **Decide canonical location**
   ...
   - Check existing "canonical" files first:
     - `js/constants.js` — general app constants.
     - `js/nostr/relayConstants.js` — relay-related constants.
     - `js/nostr/cachePolicies.js` — cache-related constants.
   - If a constant with an appropriate name already exists and fits the meaning -> **use it**.
     - Example: if `FAST_RELAY_TIMEOUT_MS` exists in `js/relayManager.js` or `relayConstants.js`, import it.
   - If none exists:
     ...
     - Add the new constant to the chosen file as a named export:
       ```js
       // js/constants.js (or appropriate file)
...
5) **Replace duplicates with imports**
   - For each occurrence:
     - Replace the numeric literal with the imported constant in minimal edit:
       ```diff
       - const TIMEOUT = 5000;
       + import { FAST_RELAY_TIMEOUT_MS } from '../nostr/relayConstants.js';
...
7) **Document decisions**
   - For each candidate, add an entry to `DECISIONS.md`:
     ...
     - Example:
       ...
       - 5000 ms used for fast relay timeout:
         ...
         - Location: js/nostr/relayConstants.js
         - Reason: relayManager & nostr-related code; existing relay constants live here.
       ```
...
8) **Commit & PR**
   ...
   - Commit changes with prefix `[constants]`:
     ...
     ```
   - Push and open PR targeting `unstable`:
     - Title: `[constants] extract FAST_RELAY_TIMEOUT_MS (5000) → js/nostr/relayConstants.js`
     - Body:
       - Short summary of change
       - Files changed
       - Commands run (`npm run lint`, `npm run test:unit`) and the relevant `TEST_LOG.md` excerpt
       - Manual QA steps
...
9) **Follow-up**
   ...
   - If any replacement surfaced ambiguous semantics that required a behavior change, document in `DECISIONS.md` and open an Issue for broader discussion.
...
* Run `npm run lint` and `npm run test:unit`.
* Commit as `[constants] extract FAST_RELAY_TIMEOUT_MS to js/nostr/relayConstants.js`.
...
* `CONTEXT.md`, `TODO.md`, `DECISIONS.md`, `TEST_LOG.md` included and accurate.
* Commit messages start with `[constants]` and PR targets `unstable`.
...
Begin now: checkout `unstable`, run discovery searches/AST scan, produce `perf/constants-refactor/candidates.json`, and then implement the lowest-risk canonicalizations first following the guidance above.

...
function loadManifestHashedPaths() {
  if (!fs.existsSync(MANIFEST_PATH)) {
    console.warn(`[lint:assets] Missing ${MANIFEST_PATH}. Skipping asset reference check (build required).`);
    return null;
  }
...
function main() {
  ...
  if (missingHtml.length > 0) {
    console.warn(`[lint:assets] Missing built HTML files: ${missingHtml.join(', ')}. Skipping check.`);
    return;
  }
